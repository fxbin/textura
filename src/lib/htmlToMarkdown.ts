...
// @ts-expect-error - package types do not expose the plugin export shape used here
import { gfm } from 'turndown-plugin-gfm';

interface TurndownImageNode {
  alt?: string;
  src?: string;
  title?: string;
}
...
function isIDEFormattedHTML(htmlData: string, textData: string): boolean {
  ...
  for (const signature of ideSignatures) {
    if (typeof signature === 'function') {
      if (signature(htmlData)) {
        matchCount++;
      }
      continue;
    }

    if (signature.test(htmlData)) {
      matchCount++;
    }
  }
